//Program for neural network classifying bank customers by risk level
//Requires NeuralNetworkBankDriver.java in same directory, and training, validation, and test files.

import java.io.*;
import java.util.*;

//Neural network
public class NeuralNetworkBank
{
    /*************************************************************************/
    ...
    int seed)
    {
        ...
        for (int i = 0; i < numberOutputs; i++)
            thetaOut[i] = 2*rand.nextDouble() - 1;	//range -1 to 1
    ...
    public void testData(String inputFile, String outputFile) throws IOException
    {
         ...
         for (int i = 0; i < numberRecords; i++)
         {
             ...
             for (int j = 0; j < numberOutputs; j++) {
            	 //convert output data
            	 outFile.print(NeuralNetworkBankDriver.deconvertClass(output[j]) + " ");                 
             }
             outFile.println();
         }
